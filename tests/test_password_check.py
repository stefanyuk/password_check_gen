...
def test_create_parser():
    assert isinstance(password_checker.create_parser(), argparse.ArgumentParser)
...
def test_check_password_without_digit(password):
    assert 'digit' not in password_checker.check_password(password)
...
def test_check_password_without_punctuation_char(password):
    assert 'punctuation_letter' not in password_checker.check_password(password)
...
def test_check_password_without_upper_lower_char(password):
    assert 'lower_upper' not in password_checker.check_password(password)


@pytest.mark.parametrize('password', ['kWU*DeW5%9NMsE', "sbr1J'sC|d/~C`", '"1qV^.YG/ScM]/M'])
@patch('builtins.print')
@patch('src.password_checker.argparse.ArgumentParser.parse_args')
